...
class CutFlowAlg(pyframe.core.Algorithm):
    ...
    def finalize(self):
        ...
        h_cutflow = self._cutflow.make_hist("cutflow_" + self.key)
        self.hists[h_cutflow.GetName()] = h_cutflow
        h_cutflow_raw = self._cutflow.make_hist_raw("cutflow_raw_" + self.key)
...
class SelectorAlg(pyframe.core.Algorithm):
    ...
    #__________________________________________________________________________
    def execute(self, weight):
        self.store[self.key_out] = []
        for part in self.store[self.key_in]:
            select = True
            for req in self.requirements:
                if not (eval(req)):
                    ...
                    break
            if select:
                self.store[self.key_out].append(part)
